"""
Hair Color Classification with DeiT-Base / DeiT3-Base
Usage:
    python classify_image_deit_base_hair_color.py --image path/to/image.jpg --checkpoint checkpoints_hair_color/best_model.pth
    python classify_image_deit_base_hair_color.py --folder path/to/folder/ --checkpoint checkpoints_hair_color/best_model.pth
"""
...
def classify_single_image(image_path, checkpoint_path, data_dir, model_name):
...
def classify_folder(folder_path, checkpoint_path, data_dir, model_name):
...
def main():
    parser = argparse.ArgumentParser(description='Hair Color Classification with DeiT')
    ...
    parser.add_argument('--model', type=str, default='deit3_base_patch16_224',
                       choices=['deit_base_patch16_224', 'deit3_base_patch16_224'],
                       help='Model architecture (default: deit3_base_patch16_224)')
    ...
    if args.image:
        if not os.path.exists(args.image):
            ...
            return
        classify_single_image(args.image, args.checkpoint, args.data_dir, args.model)
    else:
        if not os.path.exists(args.folder):
            print(f"Error: Folder not found: {args.folder}")
            return
        classify_folder(args.folder, args.checkpoint, args.data_dir, args.model)
