...
| ConvNeXt-Tiny (IN-22k) | 28M | **TBD** |

*Training time is approximate for 80 epochs on a single NVIDIA GPU (e.g., RTX 3090)
...
**Single Image**:
```bash
python classify_image.py --image path/to/hair_image.jpg
```

Output:
```
Image: path/to/hair_image.jpg
...
```bash
python classify_image.py --folder path/to/images/
```

**Using Specific Checkpoint**:
```bash
python classify_image.py --image test.jpg --checkpoint checkpoints_convnext/best_model.pth
```
...
└── classify_image.py            # Inference script
```
